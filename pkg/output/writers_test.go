...
func TestWriteCSV(t *testing.T) {
	...
	output := buf.String()
	if !strings.Contains(output, "Org,Network,Switch,Serial,Port,MAC,LastSeen") {
		t.Error("WriteCSV() missing CSV header")
	}
	if !strings.Contains(output, "Test Org,Test Network,test-switch,S123,3,00:11:22:33:44:55,2026-02-13T10:30:00Z") {
...
func TestWriteHTML(t *testing.T) {
	...
	if !strings.Contains(output, "<th>Org</th>") {
		t.Error("WriteHTML() missing table header")
	}
	if !strings.Contains(output, "Test Org") {
